...
import TRTCCloud, { TRTCScreenCaptureSourceInfo } from "trtc-electron-sdk";
  ...
  const onGetSources = async () => {
    ...
    window.desktopCapturer
      ...
      .then((res) => {
        ...
        desktopSources.value = data
          .filter((item) => item.type === 1)
          .map(
            (item, i) =>
              ({
                appIcon: res[i]?.appIcon,
                display_id: res[i]?.display_id,
                id: item.sourceId,
                name: res[i].name,
        ...
        currentSource.value = desktopSources.value?.[0];
      });
  };
