...
import { roomService, WhiteboardDataType } from "@/utils/trtc/roomService";
...
export interface IEmitsProps {
  ...
  (event: "toggleDrawingTool", isShow: boolean): void;
  (event: "WhiteboardReady"): void;
  (event: "WhiteboardError"): void;
  (event: "WhiteboardHistoryData"): void;
}
...
export function useAction(emits: IEmitsProps, props: IProps) {
  ...
  const toggleDrawingTool = () => {
    ...
    if (!state.isShowDrawingTool) {
      ...
      interactiveWhiteboardBus.emit("openDrawingTool");
    } else {
      interactiveWhiteboardBus.emit("closeDrawingTool");
    }
  ...
  const remoteDrawing = (data: WhiteboardDataType) => {
    try {
      data && tEduBoard.value.addSyncData(data);
    } catch (error) {
      console.error("remoteDrawing 出错:", error, "原始数据:", data);
    }
  };
  ...
  const addAckData = (data: WhiteboardDataType) => {
    try {
      data && tEduBoard.value?.addAckData(data);
    } catch (error) {
  ...
  const initTEduBoard = () => {
    const sdkInfo = {
      sdkAppId: roomService.basicStore.sdk.SDKAppID,
      userSig: roomService.basicStore.generateSig(props.userInfo?.userSid)
        ?.userSig,
    };

    if (!props.userInfo?.userSid || !sdkInfo?.sdkAppId || !sdkInfo?.userSig) {
      console.log(
        "参数异常",
        props.userInfo,
  ...
  onMounted(() => {
    ...
    interactiveWhiteboardBus.on("closeDrawingTool", closeDrawingTool);

    interactiveWhiteboardBus.on("change", onChange);

    interactiveWhiteboardBus.on("action", onAction);

    interactiveWhiteboardBus.on("lineColor", (val) => {
      tEduBoard.value && tEduBoard.value.setBrushColor(val);
    });

    interactiveWhiteboardBus.on("lineSize", (val) => {
      tEduBoard.value && tEduBoard.value.setBrushThin(val * 10);
    });
  });
